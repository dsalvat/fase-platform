...
/**
 * Schema for linking a KeyPerson to a BigRock
 */
export const linkKeyPersonToBigRockSchema = z.object({
  keyPersonId: z.string().uuid("ID de persona clave inválido"),
  bigRockId: z.string().uuid("ID de Big Rock inválido"),
});

/**
 * Schema for creating an inline KeyPerson (during BigRock creation)
 */
export const inlineKeyPersonSchema = z.object({
  firstName: z
    .string()
    .min(2, "El nombre debe tener al menos 2 caracteres")
    .max(50, "El nombre no puede exceder 50 caracteres"),
...
export type LinkKeyPersonToBigRockInput = z.infer<typeof linkKeyPersonToBigRockSchema>;
export type InlineKeyPersonInput = z.infer<typeof inlineKeyPersonSchema>;
